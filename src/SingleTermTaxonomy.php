...
if ( ! class_exists( 'WPS\Taxonomies\SingleTermTaxonomy' ) ) {
	...
	class SingleTermTaxonomy {
		...
		/**
		 * SingleTermTaxonomyWalker object
		 * @since 0.1.0
		 * @var object
		 */
		...
		public function __construct( $tax_slug, $post_types = array(), $type = 'radio', $default = array() ) {
			...
			$this->input_element = in_array( (string) $type, array(
				'radio',
				'select'
			) ) ? $type : $this->input_element;
			...
			// Handle bulk-editing
			if ( isset( $_REQUEST['bulk_edit'] ) && 'update' == strtolower( $_REQUEST['bulk_edit'] ) ) {
				$this->bulk_edit_handler(); // Input var ok.
			}
		}
	...
	public function select_open() {
		...
		public function walker() {
			...
			$this->walker = new SingleTermTaxonomyWalker( $this->taxonomy()->hierarchical, $this->input_element );
